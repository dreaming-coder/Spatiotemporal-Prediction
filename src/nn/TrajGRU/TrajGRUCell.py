r"""
TrajGRUCell 的实现,卷积核尺寸需要根据图片尺寸计算，必须保证根据公式算出的 feature map 尺寸是整数，不能取整
"""
...
def warp(input: Tensor, flow: Tensor) -> Tensor:
    device = input.device
    B, C, H, W = input.size()
    # mesh grid
...
class TrajGRUCell(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, kernel_size: int = 5, L: int = 5):
        ...
        same_padding = 1 if kernel_size == 3 else 2
        self.l = L

        # x to flow
        self.gammma_x = nn.Conv2d(in_channels, out_channels=32, kernel_size=5, padding=2, stride=1)

        # h to flow
        self.gammma_h = nn.Conv2d(hidden_channels, out_channels=32, kernel_size=5, padding=2, stride=1)

        # generate flow
        self.generate_flow = nn.Conv2d(in_channels=32, out_channels=self.l * 2, kernel_size=5, padding=2, stride=1)

        self.W_xz = nn.Conv2d(in_channels=in_channels, out_channels=hidden_channels,
                              kernel_size=kernel_size, padding=same_padding, stride=1)
    ...
    def forward(self, x: Tensor = None, h: Tensor = None) -> Tuple[Tensor, Tensor]:
        ...
        if x is None:
            x = torch.zeros(h.size(0), self.in_channels, h.size(2), h.size(3)).to(h.device)

        if h is None:
            h = torch.zeros(x.size(0), self.hidden_channels, x.size(2), x.size(3)).to(x.device)

        flows = self.__flow_generator(x, h)
        warped_data = []
        for flow in flows:
            warped_data.append(warp(h, flow))
